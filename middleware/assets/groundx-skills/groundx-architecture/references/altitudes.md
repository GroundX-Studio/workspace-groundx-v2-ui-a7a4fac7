...
A 6-pod subsystem (`workspace-api`, `workspace-workspace`, `workspace-provision`, `workspace-command`, `workspace-publish`, `workspace-cleanup`) reached directly by agent surfaces — internal managed-project publish workflow is the primary caller — through `workspace-api`. NOT orchestrated from `pre-process`. See `workspace-architecture.md`  for depth.
