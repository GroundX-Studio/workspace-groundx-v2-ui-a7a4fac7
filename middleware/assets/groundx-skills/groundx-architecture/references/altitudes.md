...
GroundX runs as a set of **stateless** services connected by a queue layer (all state lives in the shared data-store tier; every component horizontally scalable). The canonical customer-deployable deployment is the **Helm chart on Kubernetes**, where every component is a pod. The GroundX-managed hosted cloud service is mixed across AWS Lambda, Kubernetes, and dedicated EC2: most hosted API/event paths are Lambda; hosted file processing is dedicated EC2; extraction and managed workspace run in Kubernetes; layout and search/ranker run on dedicated EC2 hosts. A **legacy Lambda pipeline** remains in maintenance to support WordPress plugins. Backing services are selected per deployment via `values.yaml`. Common backing-service pairings (the Helm chart ships `values.yaml` examples for AWS-managed, Azure, **Red Hat OpenShift** (partnership), generic on-prem, minikube, and air-gapped targets — the list is open-ended, not enumerative):
...
| **GroundX Dashboard** | `groundx-middleware` proxy → `groundx` | `ws.eyelevel.ai` (partner-tier) | Modern proxy pattern; conceptually similar to what Agent Harness scaffolds produce (Dashboard predates the Harness) |
