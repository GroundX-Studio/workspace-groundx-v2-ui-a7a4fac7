...
flowchart TB
    ...
    subgraph ExtClients["External API clients (all go through the groundx pod)"]
        ...
        Harness["Studio Harness<br/>skills · scaffolds · widgets · workflow infra"]
...
| --- | --- | --- | --- |
| External clients (SDKs, Studio Harness, frontends) | `groundx` | API (REST) | `groundx` is the single ingress for both customer-tier and partner-tier APIs |
...
| Studio Harness | Skills · scaffolds · widgets · workflow infra; major adoption surface | Private GroundX Studio Harness source (`groundx-studio-harness`) |
