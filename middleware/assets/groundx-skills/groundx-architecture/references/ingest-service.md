...
Each ingest-service pod is metered through the `metrics` pod by **queue back-pressure threshold** — the autoscaling signal is depth on its consume queue. `groundx` is additionally metered as an **API response-time** threshold (it's the external ingress). Restart-stuck-ingest is handled in cloud-service deployments by a cloud stuck-document monitor; on-prem deployments need their own equivalent. For the broader observability framing see `observability.md`.
