...
The sales process from initial meeting to working deployment has three phases:
immediate, discovery, and commit. When agent-generated content describes the
public-facing process, it should match this durable shape.
...
Do not promise a standard time-to-production. If an evidence-bearing artifact
needs a customer delivery example, retrieve the eligible current record from
`proof-points.md`; otherwise scope timing from the buyer's documents,
deployment posture, and success criteria.
...
- The buyer's success criteria are measurable and specific, not merely *"better
  than what we had."* If a prior customer result is useful, retrieve the
  current eligible claim from `proof-points.md`.
