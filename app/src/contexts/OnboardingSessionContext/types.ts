...
/**
 * `save-schema-gate-handoff`: optional discriminator carried on the
 ...
 * sign-in succeeds. `save-schema` is the schema-design Save → sign-in →
 * persist → ingest-picker handoff path; future causes can be added
 * without changing the gate API.
 */
...
export interface OnboardingSessionState {
  ...
  /** Active scenario, set when user picks a sample in the Ingest picker. */
   ...
   * pre-selection. Cleared whenever the user leaves the builder surface.
   */
...
export interface OnboardingSessionApi {
   ...
   * destination VIEWER STEP, WITHOUT pushing it (frame-free journey-progress).
   * The orchestrator's
   ...
   * `attachedSchema` rides onto the picker step (the schema-design Save →
   * sign-in → persist → picker hand-off). Onboarding-only.
   */
