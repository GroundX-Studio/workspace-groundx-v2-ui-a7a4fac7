...
const isFunctionalStorage = (value: unknown): value is Storage => {
  try {
    // Require a *genuine* Storage, not merely an object with clear/setItem.
    // Node 24+ ships a native global Storage; on some versions (e.g. 26) its
    // sessionStorage is functional but the wrong "brand" — not `instanceof`
    // jsdom's Storage. Accepting it left `instanceof Storage` false in tests.
    // Insisting on a real Storage makes the shim swap in the in-memory stand-in
    // (which chains to Storage.prototype, so it IS instanceof Storage)
    // consistently across the supported Node range.
    return (
      typeof globalThis.Storage === "function" &&
      value instanceof globalThis.Storage &&
