...
| Uncommitted draft Template (session-scoped) | the in-progress edit, resolved to a `TemplateSaveInput`-shaped object | `TemplateSaveInput` (`@groundx/shared`, nullable name uncommitted) | `chat_session_entities.draft_template_json` | a `TemplateSaveInput` | shared `templateSaveInputSchema`; additive `ALTER TABLE` reconcile; hydrated by `viewerFromActiveEntity`; `persistedColumnPolicy` guard; orthogonal to the committed-Template lifecycle (`template-lifecycle` delta); excluded from the committed save |
