...
|---|---|---|---|---|---|
| Citation | `Citation` (re-export `@groundx/shared`) | `Citation` (re-export `@groundx/shared`) | `chat_messages.citations_json` | array of `Citation` | one shared Zod schema (`regions:{page,bbox,tier}[]` + legacy first-region `page?/bbox?/tier?` alias, `multi-region-citations`); `parseCitations`+`normalizeCitation` sanitize + keep both shapes in sync at the hydrate boundary. Producers: snippet-quote verify loop AND extraction-sourced entries (model emits `{documentId, field, value}`; middleware validates against the real extract payload). Resolves MULTI-region geometry via the X-Ray match (numeric for numbers / normalized whole-token for words; EVERY occurrence; exact-duplicate boxes deduped). NEVER drops a validated grounding: a container descends to its scalar leaves; an unverified quote → whole-page `ambient` marker; an unlocatable validated value → label-located region or a regionless `ambient` chip. Only fabricated entries (bad docId/path/value) drop. Render-time: adjacent same-tier boxes merge for display |
...
| Seeded default report template (report-default-template) | `SAMPLE_REPORT_TEMPLATE_ID` (shared const) → `scenario.manifest.reportTemplateId` → `reportOverlay.templateId`; client read `getReportTemplate(id) → {template, owned}` (`api/smartReport.ts`) | `seedSampleReportTemplate` + `SAMPLE_TEMPLATE_OWNER` sentinel (`db/seedSampleProject.ts`, boot); `GET /api/widgets/smart-report/reports/template/:id` (access-scoped read) + the POST-render save-path ownership guard (403 `not_template_owner`) | ONE `templates` row (`kind:"report"`, `groundx_username = SAMPLE_TEMPLATE_OWNER`, well-known id) | `body_json` = `ReportBody` (section defs) | shared `SAMPLE_REPORT_TEMPLATE_ID`; idempotent boot seed; read endpoint anon-scopes to public/sample-owned; `owned:false` drives builder fork-on-edit; ownership-enforced save (spec: smart-report ADDED reqs) |
...
| RewriteItem (agentic-template-item-editor) | `RewriteItemRequest`/`RewriteItemResult` + `TemplateItemKind` + `ReportSectionItem` (re-export `@groundx/shared`); client `api/templateItem.ts` | same; `POST /api/template-item/rewrite` (+ `POST /api/report-section/preview` returns `RenderedSection`) | — (no persistence) | — | one shared Zod (`rewriteItemRequestSchema`/`rewriteItemResultSchema`, discriminated on `kind`); rewrite `item` reuses `templateFieldSchema`/`reportSectionItemSchema`; name held fixed structurally; rewrite is user-invoked, NOT in `SERVER_TOOL_CATALOG` |
| ItemPreview (transient) | in-memory ChatStore `itemPreviews: Map<string, ExtractedFieldValue \| RenderedSection>` (seeded + added items) | — | — (NOT persisted) | — | reuses the shared result specializations; recomputed on demand; cleared on document-scope change; replaces the `addedFields`-only `setSchemaFieldExtraction` path |
| Uncommitted draft Template (per-entity) | `EntitySession.draftTemplate` — the in-progress question set, resolved to a `DraftTemplate` (= `TemplateSaveInput` with a NULLABLE name — uncommitted has no name yet) | `DraftTemplate` / `draftTemplateSchema` (`@groundx/shared`) | `chat_session_entities.draft_template_json` | a `DraftTemplate` (also mirrored to the localStorage `SerializedEntitySession.draftTemplate`) | shared `draftTemplateSchema`; additive `ALTER TABLE` reconcile; written at editor SAVE-MOMENTS via the atomic `commitDraftTemplate` action (flattens overlay→draft base + resets overlay); WINS as the base schema on load (Extract `draftBodyToSchemaDef`); `persistedColumnPolicy` now guards `chat_session_entities`; orthogonal to the committed-Template lifecycle; excluded from the committed save |
...
| `Citation` / `CitationTier` / `CitationSourceRegion` | re-exports `@groundx/shared` (`types/onboarding.ts`) | `documentId, regions:{page,bbox,tier}[], page?, bbox?, snippet?, confidence?, tier?, answerSpan?` — `regions` is canonical; `page?/bbox?/tier?` is the derived first-region alias (`page` optional for the regionless "location unknown" chip); `citationRegions(c)` helper = the one reader (`multi-region-citations`) | `CiteChip`, chat, extract, report |

> **Citation philosophy (locked 2026-06-14).** A citation shows **ALL the source
> material that informed an answer or extracted value** — every place the cited
> value/quote appears in the documents — **not a single "best" source.** Showing
> everything considered (incl. a value that was considered but may have been the
> wrong pick) lets a reader verify what the answer was built on and catch a
> mistake. Consequences: (1) highlight EVERY occurrence of a cited value (value
> match; do NOT narrow to one instance via the `field` path NOR the field label);
> (2) precision is **per highlight** (each region its own `tier`), not per citation;
> (3) locate a NUMBER by NUMERIC comparison (parse both sides, whole numeric token:
> `7613.2`==`7,613.20`==`$7,613.20`; reject `18.43`≠`18.44` and `18.43`∉`118.437`)
> and a WORD by normalized whole-token match — NOT raw substring, NOT fuzzy, NOT
> embeddings (embeddings verify a paraphrased *quote* only); (4) schema-agnostic (no
> hardcoded field names); (5) never drop a real grounding — a validated value not
> printed findably (reformatted/derived/too-common) degrades to a label-located or
> regionless `ambient` chip ("location unknown"), and an unverified quote degrades to
> a whole-PAGE `ambient` marker; still reject fabricated ones. Plan: OpenSpec
...
| `AppMode`/`Scenario`/`JourneyStage`/`GateTrigger`/`AuthState` | unions | literal sets | shell, session, nav. `JourneyStage` (shared) is the frame-free onboarding journey-progress axis (replaced the retired `FFrame` literal union); the canvas surface is the active `ViewerStep` kind, mapped to a stage via `journeyStageForStepKind` |
...
| `ScenarioManifest` | composes `ScenarioHero` | `id, hero, thinkingScript[], chatSeeds[], sampleChatScript[], extractionSchema?, reportTemplateId?` (+ `sampleExtractionValues`) | scenario fixtures. `reportTemplateId?` (report-default-template) = the seeded default report template id this scenario loads onto `reportOverlay.templateId` (utility → `SAMPLE_REPORT_TEMPLATE_ID`; loan/solar omit → empty state); config-driven, NO scenario hardcoded in the shell. NOTE: `extractionSchema` / `sampleExtractionValues` / `sampleChatScript` are the mock-data path; superseded by real-data retrieval via `getGroundXWorkflow(filter.workflow_id)` (schema) + `getGroundXDocumentExtract(id)` (values) — see `docs/agents/real-data-rewire-gap.md`. |
...
| `ViewerStep` | union | `doc-viewer{documentId,page?,highlight?}` · `extract-workbench{scenarioId,focusedCategoryId?,surface?:"fields"\|"design"}` · `interact-chat{documentId?}` · `report{surface?:"render"\|"builder",selectedSectionId?}` · `integrate` · `ingest-picker` | canvas surface. `extract-workbench.surface:"design"` is the schema-design sub-position (reached via `editSchema`, steady-reachable — mirrors `report.surface:"builder"`) |
...
| `PendingReportOverlay` | = `PendingTemplateOverlay<ReportSectionItem,ReportSectionEdit,ReportSectionProposal>` + `templateId?: string` | (alias) | Report builder overlay (`reportOverlay` slot); `addReportSection`/`editReportSection`/`removeReportSection` actions; SmartReportBuilder (f4a). `templateId?` = the report template the render/builder targets (the render-surface + routing source of truth); WRITERS: `setReportTemplateId(id\|undefined)` (report-default-template — onboarding-bootstrap loads the scenario's seeded default) and the `pinToReport({…,templateId?})` pin path. NEVER a client scope→fixture map. |
...
|---|---|---|---|
| CanvasOrchestrator | `CanvasIntent` (real union) | kinds: `showSample, openDocument, highlightCitation, showCitations, jumpToPage, showExtract{scope,schemaId,focusedCategoryId?}, editSchema{schemaId}, showInteract{scope}, showReport{templateId,scope}, editTemplate, showIntegrate{scope}, openGate, proposeSchemaField, accept/rejectSchemaField, commitGate, dismissGate, openBookCall, presentExperienceBeat{beat}, …` | `editSchema` flips the `extract-workbench` step to `surface:"design"` (real outcome, both experiences); `presentExperienceBeat` (`llm:false`) carries an extensible `beat` discriminator (`ingest-picker` return-to-picker + entity-deactivate; `understand-scanning` snap) — the one experience-internal seam that replaced the deleted frame transitions. + `StampedIntent{intent,source,intentId}`, `CanvasAdapter<K>{kind,apply}` |
...
| `EntitySession` | — | `kind, id, lastStep (PersistedViewerStep), reachedStages (Set<JourneyStage>), createdAt, lastVisitedAt` | per-entity journey. `lastStep` = the resume anchor (kind + navigational payload, restored verbatim — replaced the frame-keyed resume field); `reachedStages` = the non-contiguous SET of journey stages each individually reached, drives checkmarks only (replaced the frame-keyed completion array) |
| OnboardingSession | `OnboardingSessionState` | `gate{status,trigger,cause?}, scenario` | gate lifecycle (LC3). No frame field — the canvas surface is the active `ViewerStep`; journey-progress (resume anchor + reached-stage set) lives on `EntitySession`, layered on by the onboarding shell |
...
| `GetReportTemplateResult` / `ReportTemplateDefinition` | — | `{template:{id,name,sections[]}, owned:boolean}` | report-default-template — `getReportTemplate(id)` (`api/smartReport.ts`, in the `report` client group); anon-readable `GET /api/widgets/smart-report/reports/template/:id` (access-scoped: anon → public/sample only; `404`→`null`). `owned` drives the builder's fork-on-edit. |
...
| union-typed DB columns | SQL | the 3 genuinely-closed unions — `chat_messages.role`, `viewer_events.action`/`source`, `intent_log.source` — are bare `VARCHAR` at the DB but **Zod-enum-COERCED** in the row mappers via `coerceEnum(schema, value, fallback)` (§4c, 2026-05-31): a corrupt value coerces to a safe in-union default, not blind-cast. `entity_key`/`intent_kind` are FREE strings (open discriminators), correctly NOT coerced | row-mapper coercion is the union guard (no DB CHECK) |
...
| `SAMPLE_REPORT_TEMPLATE_ID` | shared const (`@groundx/shared`) | `"rt-sample-utility-bill"` | report-default-template — the one well-known id for the seeded default report template; imported by the middleware seed (`seedSampleReportTemplate`), the utility scenario manifest (`sampleScenarios.ts`), and test infra (`makeFakeApi`/`scenarioFixtures`). The owner sentinel `SAMPLE_TEMPLATE_OWNER` lives middleware-side (`db/seedSampleProject.ts`) — it is the public marker (the `templates.groundx_username` is NOT NULL, no public flag). |
