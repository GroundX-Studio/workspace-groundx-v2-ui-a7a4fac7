...
- **New frontend network function** → add it to the injected `Api` client (`app/src/api/client.ts`) and let tests override it through `makeFakeApi` / render-harness `api` options. Runtime consumers use `useApi()`; tests do not add per-file `vi.mock("@/api/...")` for migrated boundaries. This mirrors middleware DI (`createApp({ repository, partnerClient, groundxClient, llmClient, scenarioRegistry })`). **Guard: `frontend-api-injection-guard.test.ts`** fails direct imports or per-file mocks for the migrated session/chat/auth boundaries.
