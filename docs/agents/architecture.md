...
| A new CanvasIntent | extend the discriminated union (`canvasIntentSchema` in `@groundx/shared`) → register an adapter via `registerAdapter` OR add a built-in handler in `dispatch`. **If it's a navigation, classify its surface in `CANVAS_NON_DOC_NAV_INTENT_KINDS` (`@groundx/shared`) — the ONE place surface classification lives** (the chat auto-highlight guard reads it; `satisfies` pins entries to real kinds). Don't re-hardcode a nav/surface set per consumer (principle 6) |
