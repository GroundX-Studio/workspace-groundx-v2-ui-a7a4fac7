...
| `https://docs.groundx.ai` | "Docs" CTA | `app/src/views/Onboarding/OnboardingShell.tsx:639`, `app/src/views/Scoped/ScopedConversationShell.tsx:139` | ❌ Hardcoded URL | **Gap** — add `VITE_DOCS_URL` env or `APP_CONFIG.docs.url`; on-prem deploy sets it to internal docs site OR hides the CTA via `APP_MODE_PRESET` |
| `APP_CONFIG.calendly.url` (`VITE_CALENDLY_URL`) | "Book a call" viewer scheduler URL | `app/src/appConfig.ts:148` | ✅ env-var overridable; may be blank | Set `VITE_CALENDLY_URL` to an approved scheduling URL, or leave unset and hide/replace the CTA for air-gapped deploys |
| `https://assets.calendly.com` | Calendly widget script + stylesheet | `app/src/lib/calendlyEmbed.ts:1`, `middleware/src/app.ts:242`, `middleware/src/app.ts:270` | ⚠️ Hardcoded official Calendly asset host | If an on-prem deploy cannot reach Calendly, replace the scheduler provider/helper and remove this origin from CSP |
| `https://calendly.com`, `https://*.calendly.com` | CSP frame-src + scheduled-event origin trust | `middleware/src/app.ts:243`, `app/src/lib/calendlyEmbed.ts:21` | ⚠️ Hardcoded official Calendly trust boundary | Keep only for Calendly-hosted scheduling; a non-Calendly scheduler needs provider-specific frame and postMessage trust rules |
...
- Proper env-var seams already exist for GroundX REST, LLM providers,
  telemetry providers, and the public Calendly scheduling URL. The
  telemetry trio (PostHog / Sentry / GA) follows the same "no-op when
  unset" pattern from CF-13 / OB-02 / OB-03.
- Remaining hardcoded URL gaps before a serious on-prem deploy:
  `docs.groundx.ai`, the two font `@import`s (Inter via Google,
  Thicccboi via Fontshare), and the eyelevel.ai terms URL.
- CSP allowlist hardcodes for fonts + Calendly are a derived concern.
  Fonts should be removed when fonts are self-hosted; Calendly origins
  should be removed if the scheduler CTA is hidden or swapped to an
  internal scheduling provider.
...
2. **Scheduler provider switch / hide affordance.** `VITE_CALENDLY_URL`
   now feeds `APP_CONFIG.calendly.url`; on-prem still needs a policy
   choice: internal scheduler adapter or hide the "Book a call" CTA.
