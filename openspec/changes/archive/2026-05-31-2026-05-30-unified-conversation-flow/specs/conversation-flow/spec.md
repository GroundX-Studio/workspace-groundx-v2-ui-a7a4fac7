# Spec Delta — conversation-flow (NEW capability)

The durable home for the single chat conversation flow + the pluggable directed-experience layer.
Replaces the per-mode forked flow components with one engine + one view; onboarding becomes one
optional, composed experience (no mode, no entry context).
