# Spec Delta — smart-report (NEW capability)

The durable home for the Report chapter (S3 render + S3a builder). Consolidates the W8 contract
(`project_dev_contracts.md`) and the report tool surfaces previously scattered in `agent-tools`.
v1 target = the **Utility single-document** report; built **doc-count-agnostic** for Solar later.
