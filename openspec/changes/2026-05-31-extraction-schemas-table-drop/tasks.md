...
- [x] **GATE 1 — UNBLOCK (soak):** Confirm the `templates` migration has been LIVE in production
...
- [x] **Adversarial review:** the gate evidence must be REAL, not asserted. Re-run
...
- [x] **GATE 2 — SEQUENCING (after the audit):** Confirm `2026-05-31-e2e-experience-audit` has
...
- [x] **Adversarial review:** confirm the audit PASS is real, not anticipated — the
...
- [x] **Failing-first test:** Re-target the migration test in
...
- [x] **Adversarial review:** open the test and run it — confirm it is genuinely RED right now
...
- [x] **Remove the copy-migration:** Delete the `INSERT INTO templates … SELECT … FROM
...
- [x] **Remove the orphan CREATE:** Delete the now-unreferenced
      ...
      sweep.
- [x] **Drop the table:** Add the `DROP TABLE IF EXISTS extraction_schemas` migration step so an
...
- [x] **Adversarial review:** all three must be removed/added TOGETHER — a partial sweep is the
...
- [x] **Test GREEN:** the re-targeted migration test passes — fresh boot + `createSchema()` succeed
      with the table gone.
- [x] **Adversarial review:** confirm GREEN is earned by the code change, not by retargeting the
...
- [x] Migration test green; `scaffold/middleware` suite green (run with `--no-file-parallelism`
      per the file-serial config) and `scaffold/app` suite green.
- [x] `npm run build` (app + middleware) green; widget-contract + no-hardcoded-styles +
      tool-quality drift guards green.
- [x] `openspec validate 2026-05-31-extraction-schemas-table-drop --strict` green; no delta vs
      shipped/archived specs beyond this change's intended one.
- [x] Update `docs/agents/data-model.md` to drop `extraction_schemas` from the persistence facts
      table (it no longer exists).
- [x] **Adversarial review of the WHOLE change against the plan AND the real code:** both gates'
