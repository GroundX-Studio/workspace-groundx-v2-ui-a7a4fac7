...
- [x] **Repro test (failing first).** Add a steady-shell integration test
...
- [x] **Fix.** Replace the empty `<Box>` canvasPane in `ScopedConversationShell.tsx` with
...
- [x] **Re-verify live.** With the dev harness on REAL GroundX, drive `/workspaces`:
...
- [x] **Adversarial review (before done).** (a) Revert the canvasPane change → confirm the
      ...
      green.
- [x] **Close the audit linkage.** Flip e2e-audit DL-5 to `triaged → fixed → reverified`
