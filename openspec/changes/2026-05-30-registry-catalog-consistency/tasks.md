...
- [ ] **FIRST — decide `toolRegistry`'s fate (it may be an orphan).** Audit (2026-05-30) found the app `toolRegistry` singleton + every widget `handler` has **zero production importers** — the live LLM catalog is the middleware `SERVER_TOOL_CATALOG` (`toolsForStep`, `chatRouter.ts:575`), and the app dispatches server-built `reply.intents`. Do NOT align an orphan to `Catalog<T>` blindly. Decide + record: (a) it IS the intended source of truth → wire a real consumer (and reconcile with the `SERVER_TOOL_CATALOG` mirror, widget-role-access's concern); or (b) it's dead → delete it (and the dead app-side `category`/`handler` duplication of the server `intentBuilder`). Only the surviving registries get the `Catalog<T>` treatment below.
- [ ] **Failing test:** `toolRegistry` satisfies `Catalog<WidgetTool>` — `byId(name)` returns the same tool as `byName(name)`; `all()` unchanged; `forStep` untouched; duplicate id still throws at boot. *(Only if the decision above keeps it live.)*
- [ ] Add `byId` to `ToolRegistry` (alias of `byName`; document that a tool's id is its `name`); declare `ToolRegistry extends Catalog<WidgetTool>`. Refactor the registry's existing bespoke duplicate-name throw to call the shared `assertUniqueIds`, passing `sourceOf` = each tool's module path — ONE mechanism for the invariant, not two, and it preserves the existing "declared in two modules" semantics by naming the colliding modules.
- [ ] **Failing test:** `ScenarioRegistry` exposes `all()` returning `state.scenarios`; `byId` unchanged; status/`refresh` untouched.
- [ ] Add `all()` to `ScenarioRegistryApi`; have the ready-state data view satisfy `Catalog<ScenarioConfig>` (async wrapper stays).
