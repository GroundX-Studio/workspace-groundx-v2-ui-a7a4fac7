...
- [x] 1.1 `ViewerWidgetFrame` body `flexDirection: column` (TDD) — failing test: a widget
...
- [x] 1.2 Two composable layers under `components/primitives/` (TDD):
...
- [x] 1.2a Visual design pass (chrome-devtools MCP) — render the primitive in the running app
...
- [x] 1.3 Anti-flash delay in the boundary (TDD) — an optional appearance delay (~150ms default)
...
- [x] 1.4 Wrap `PdfViewerWidget.tsx` (TDD) — replace the silent empty box
...
- [x] 1.5 Wrap `Extract.tsx` (TDD) — replace the `!schema` body loader ([Extract.tsx:645]) with the
      ...
      mid-pane.
- [x] 1.6 Wrap `SmartReportRender` (first-paint state); confirm `Integrate` (no load) renders no
      mark (TDD).
- [x] 1.7 Wrap `BookCallView.tsx` + `SignUpWidget.tsx` (TDD) — the boundary for the blocking
...
- [x] 1.8 Retire `LoadingDots` at the chat + gate + status-band sites (TDD). By this point
...
- [x] 1.9 Universality drift guard (TDD) — a test asserting every ENUMERATED loading/thinking
...
- [x] 2.1 Live preview verification (chrome-devtools MCP) — confirm the shared green mark appears
...
- [x] 2.2 Full suite + typecheck green; `OPENSPEC_TELEMETRY=0 openspec validate --all --strict`;
      adversarial review vs. this proposal AND the real code (not the seam).

## Verification record (2026-07-07)

- 1.2a/2.1 live (chrome-devtools, dev stack, fetch-delayed to hold the state): the `lg` boundary
  measured on the Extract pane — brand green core rgb(161,236,131), 1.8s breathe animation
  running, ring layer present, centered in the 831×613 pane, "Reading the extraction…" message;
  resolved state has ZERO stray marks/boundaries; no console errors. Screenshots: this
  chrome-devtools server's Page.captureScreenshot times out (documented quirk in
  feedback_ui_verification_tooling) — DOM measurements stand as evidence per "measure, don't
  eyeball". Dark mode: N/A — the app ships a single light theme.
- 1.7: SignUpWidget has no async pre-render state (plain form) → nothing to wrap; BookCallView
  got the boundary as a pre-embed overlay (Calendly injects into a ref-managed node, so the
  boundary overlays rather than wraps; Calendly's own spinner stays suppressed).
- 1.8: chat thinking = ThinkingStream (analyze-and-chat-ux §6) with the bare `sm` mark ONLY as
  the pre-first-event fallback; gate panel/rail + frame status band on the bare mark;
  `LoadingDots/` deleted; §1.9 guard enforces all of it.
