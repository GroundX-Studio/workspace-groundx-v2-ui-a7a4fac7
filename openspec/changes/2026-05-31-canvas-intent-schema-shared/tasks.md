...
- [ ] **Failing test FIRST (app suite, exercises the shared schema):** add a
...
- [ ] Add `canvasIntentSchema` to `shared/src/index.ts` as a
...
- [ ] **Adversarial review:** diff the new Zod union variant-by-variant against
...
- [ ] **Failing/guard step:** before editing, confirm the orchestrator
...
- [ ] Replace the hand-declared `export type CanvasIntent = | {...} | ...` in
...
- [ ] **Adversarial review:** grep the app for any remaining local `CanvasIntent`
...
- [ ] **Failing test FIRST:** extend `ChatStoreContext/ChatStoreServerHydrator.test.tsx`
...
- [ ] Rewrite `coerceHydratedIntent` in `ChatStoreContext.tsx` to delegate to
...
- [ ] **Adversarial review:** open the test file — confirm the RED case is real
...
- [ ] **Failing test FIRST:** add to `mysqlRepository.test.ts` (or the existing
...
- [ ] In `mysqlRepository.ts` `rowToChatSession`, replace
...
- [ ] **Adversarial review:** confirm the WRITE side (`upsertChatSession`
...
- [ ] `openspec validate 2026-05-31-canvas-intent-schema-shared --strict` passes.
- [ ] App suite green (incl. `ChatStoreServerHydrator.test.tsx`,
...
- [ ] `npm run build` clean (app + middleware + shared) and drift guards green
      (no-hardcoded-styles, widget-contract, tool-parity unaffected).
- [ ] Adversarial review of the WHOLE change against the plan AND the real code:
      one source of truth (no rival `CanvasIntent` union remains); both read
      boundaries validate (not just one); valid intents behavior-preserved; the
      RED tests were genuinely red before. Then archive.
